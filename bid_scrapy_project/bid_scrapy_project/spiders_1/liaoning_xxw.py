...
class LiaoningXxwSpider(scrapy.Spider):
    name = 'liaoning_xxw'
    ...
    def __init__(self):
        self.infoType = {
            "招标公告": "http://218.60.147.115/gcjy/003001/about.html",
            "资格预审公告": "http://218.60.147.115/gcjy/003005/about.html",
            "中标候选人公示": "http://218.60.147.115/gcjy/003002/about.html",
            "资格预审结果公示": "http://218.60.147.115/gcjy/003003/about.html",
            "中标结果公示": "http://218.60.147.115/gcjy/003004/about.html",
        }
    ...
    def start_requests(self):
        for info_type, info_url in self.infoType.items():
            ...
            for page in range(1, 3):
                ...
                if page > 1:
                    info_url = info_url.replace("about", "{}".format(page))
    ...
    def parse(self, response):
        ...
        list = response.xpath('//li[@class="wb-data-list"]')
        for li in list:
            # 详情页链接
            contentUrl = li.xpath("./div/a/@href").extract_first()
            contentUrl = response.urljoin(contentUrl)
            # 文章标题
            title = li.xpath('./div/a/text()').extract_first().strip()
            # 文章发布时间
            times = li.xpath('./span[@class="wb-data-date"]/text()').extract_first().strip()
            # 获取当前年月日
    ...
    def getContentInfo(self, response):
        ...
        # 带有html的文本
        str_html_content = etree.HTML(response.text).xpath('//div[@class="news-article-para"]')
        if str_html_content:
        ...
        items_infos['website_name'] = "辽宁建设工程信息网"
        items_infos['website_url'] = "http://218.60.147.115/gcjy/about.html"
