...
@Author : zhangpf
@File : tyc_qualification_certificate.py
@Desc : 天眼查--资质证书
@Software: PyCharm
"""
...
from untils.urls import QUALIFICATION_CERTIFICATE

requests.packages.urllib3.disable_warnings(InsecureRequestWarning)


def create_json(pageNum, info_id, tyc_id, company_name, res_json):
    folder_name = os.getcwd() + "/jsondata/tyc_资质证书_file/"
    ...
    logger.debug(("--tyc_资质证书_file__写入到-->" + file_nm))


def get_authoriaztion(info_id, company_name, tyc_id, pageNum):
    version = "Android 12.67.0"
    url = QUALIFICATION_CERTIFICATE.format(tyc_id, pageNum)
...
def get_qualification_certificate_page(
...
def get_qualification_certificate_info(info_id, company_name, tyc_id, pageNum):
    try:
        # print(url)
        # res = requests.get(url=url, headers=headers, verify=False).text
        ...
        for ite in items_list:
            item = {
                ...
                "info_id": str(info_id),
                "certNo": ite.get("certNo", ""),
                "endDate": ite.get("endDate", ""),
                "businessId": ite.get("businessId", ""),
                "certificateName": ite.get("certificateName", ""),
                "t_id": ite.get("id", ""),
                "productNameMaster": ite.get("productNameMaster", ""),
...
def main():
    ...
    for data in data_list:
        ...
        deviceID = data["data"]["deviceID"]
        x_auth_token = data["data"]["x_auth_token"]
        pages_total = get_qualification_certificate_page(
            info_id, company_name, tyc_id, tyc_hi, Authorization, duid, deviceID, x_auth_token, initial_pageNum
        )
        if pages_total:
            print(company_name)
            for pageNum in range(1, int(pages_total) + 1):
                items = get_qualification_certificate_info(info_id, company_name, tyc_id, pageNum)
                try:
                    mq = MysqlPipelinePublic()
                    for item in items:
                        mq.insert_sql("t_zx_qualification_certificate", item)
                    mq.close()
