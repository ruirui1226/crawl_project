...
# -*- coding: utf-8 -*-
"""
@Time : 2023/5/15 13:31
@Author : zhangpf
@File : tyc_publicity_of_land_plots.py
@Desc : 天眼查---地块公示
@Software: PyCharm
"""
...
from untils.urls import PUBLICITY_OF_LAND_PLOtS

requests.packages.urllib3.disable_warnings(InsecureRequestWarning)


def create_json(pageNum, info_id, tyc_id, company_name, res_json):
    folder_name = os.getcwd() + "/jsondata/tyc_地块公示_file/"
    ...
    logger.debug(("--tyc_地块公示_file__写入到-->" + file_nm))


def get_authoriaztion(info_id, company_name, tyc_id, pageNum):
    version = "Android 12.67.0"
    url = PUBLICITY_OF_LAND_PLOtS.format(tyc_id, pageNum)
...
def get_publicity_of_land_plots_page(
...
def get_publicity_of_land_plots_info(info_id, company_name, tyc_id, pageNum):
    try:
        # print(url)
        # res = requests.get(url=url, headers=headers, verify=False).text
        ...
        items_list = res_json["data"]["pageData"]
        items = []

        for ite in items_list:
            item = {
                # "id": str(tyc_id) + str(ite.get("businessId", "")),
                "info_id": str(info_id),
                "landAreaUnit": ite.get("landAreaUnit", ""),
                "publicationOrganize": ite.get("publicationOrganize", ""),
                "landUsefulness": ite.get("landUsefulness", ""),
                "landArea": ite.get("landArea", ""),
                "businessId": ite.get("businessId", ""),
                "administrativeDistrict": ite.get("administrativeDistrict", ""),
                "landLocation": ite.get("landLocation", ""),
                "publicationDate": ite.get("publicationDate", ""),
...
def main():
    ...
    for data in data_list:
        ...
        deviceID = data["data"]["deviceID"]
        x_auth_token = data["data"]["x_auth_token"]
        pages_total = get_publicity_of_land_plots_page(
            info_id, company_name, tyc_id, tyc_hi, Authorization, duid, deviceID, x_auth_token, initial_pageNum
        )
        if pages_total:
            print(company_name)
            for pageNum in range(1, int(pages_total) + 1):
                items = get_publicity_of_land_plots_info(info_id, company_name, tyc_id, pageNum)
                try:
                    mq = MysqlPipelinePublic()
                    for item in items:
                        mq.insert_sql("t_zx_publicity_of_land_plots", item)
                    mq.close()
