...
def create_json(pageNum, info_id, tyc_id, company_name, res_json):
    folder_name = os.getcwd() + "/jsondata/tyc_上榜榜单_file/"
    ...
    logger.debug(("--tyc_上榜榜单_file__写入到-->" + file_nm))


def get_authoriaztion(info_id, company_name, tyc_id, pageNum):
    version = "Android 12.67.0"
    url = ON_THE_LIST.format(tyc_id, pageNum)
...
def get_on_the_list_page(info_id, company_name, tyc_id, tyc_hi, Authorization, duid, deviceID, x_auth_token, pageNum):
    try:
        headers = {
            # """
            "x-b3-traceid-jindi": "",
...
def get_on_the_list_info(info_id, company_name, tyc_id, pageNum):
    try:
        # print(url)
        # res = requests.get(url=url, headers=headers, verify=False).text
        ...
        for ite in items_list:
            item = {
                ...
                "info_id": str(info_id),
                "gid": ite.get("gid", ""),
                "releaseDate": ite.get("hasCaseExplanation", ""),
                "businessId": ite.get("businessId", ""),
                "inTheListName": ite.get("inTheListName", ""),
                "pid": ite.get("pid", ""),
                "source": ite.get("source", ""),
                "dataJson": ite.get("dataJson", ""),
                "sourceUrl": ite.get("sourceUrl", ""),
                "issueYear": ite.get("issueYear", ""),
                "rankingType": ite.get("rankingType", ""),
                "name": ite.get("name", ""),
                "ranking": ite.get("ranking", ""),
                "detailUrl": ite.get("detailUrl", ""),
                "t_id": ite.get("id", ""),
...
def main():
    ...
    for data in data_list:
        ...
        deviceID = data["data"]["deviceID"]
        x_auth_token = data["data"]["x_auth_token"]
        pages_total = get_on_the_list_page(
