...
# -*- coding: utf-8 -*-
"""
@Time : 2023/5/23 14:22
@Author : zhangpf
@File : tyc_tax_rating.py
@Desc : 税务评级
@Software: PyCharm
"""
import sys
...
from untils.urls import TAX_RATING
...
def create_json(pageNum, info_id, tyc_id, company_name, res_json):
    folder_name = os.getcwd() + "/jsondata/tyc_历史终本案件_file/"
    ...
    logger.debug(("--tyc_历史终本案件_file__写入到-->" + file_nm))


def get_authoriaztion(info_id, company_name, tyc_id, pageNum):
    version = "Android 12.67.0"
    url = TAX_RATING.format(tyc_id, pageNum)
...
def get_tax_rating_page(info_id, company_name, tyc_id, tyc_hi, Authorization, duid, deviceID, x_auth_token, pageNum):
    try:
        headers = {
            # """
            "x-b3-traceid-jindi": "",
...
def get_tax_rating_info(info_id, company_name, tyc_id, pageNum):
    try:
        # print(url)
        # res = requests.get(url=url, headers=headers, verify=False).text
        ...
        for ite in items_list:
            item = {
                "info_id": info_id,
                "name": ite.get("name", ""),
                "idNumber": ite.get("idNumber", ""),
                "grade": ite.get("grade", ""),
                "year": ite.get("year", ""),
                "evalDepartment": ite.get("evalDepartment", ""),
                "type": ite.get("type", ""),
                "businessId": ite.get("businessId", ""),
                "taxArrearsAnnouncement": ite.get("taxArrearsAnnouncement", ""),
                "illegalTax": ite.get("illegalTax", ""),
                "taxArrearsAnnouncementData": ite.get("taxArrearsAnnouncementData", ""),
                "illegalTaxData": ite.get("illegalTaxData", ""),
                "unique_id": str(info_id) + ite.get("businessId", ""),
...
def main():
    ...
    for data in data_list:
        ...
        initial_pageNum = 1
        ex = conn.sadd("tyc_tax_rating", tyc_id)
        ...
        if ex == 1:
            ...
            deviceID = data["data"]["deviceID"]
            x_auth_token = data["data"]["x_auth_token"]
            pages_total = get_tax_rating_page(
                info_id, company_name, tyc_id, tyc_hi, Authorization, duid, deviceID, x_auth_token, initial_pageNum
            )
            if pages_total:
                print(company_name)
                for pageNum in range(1, int(pages_total) + 1):
                    items = get_tax_rating_info(info_id, company_name, tyc_id, pageNum)
                    try:
                        for item in items:
                            mq.insert_sql("t_zx_tax_rating", item)
                    except Exception as e:
                        logger.debug(e)
            else:
                pass
