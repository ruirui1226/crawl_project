...
def create_json(pageNum, info_id, tyc_id, company_name, res_json):
    folder_name = os.getcwd() + "/tyc_dishonest_file_历史失信信息/"
    if not os.path.exists(folder_name):
        os.makedirs(folder_name)
    else:
        pass
    ...
    logger.debug(("--tyc_dishonest_file__写入到-->" + file_nm))
...
def get_authoriaztion(info_id, company_name, tyc_id, pageNum):
    ...
    url = (
        f"https://api4.tianyancha.com/services/v3/past/dishonest?keyWords={company_name}&pageSize=20&pageNum={pageNum}"
    )
...
def get_dishonest_page(info_id, company_name, tyc_id, tyc_hi, Authorization, duid, deviceID):
    try:
        headers = {
        ...
        url = f"https://api4.tianyancha.com/services/v3/past/dishonest?keyWords={company_name}&pageSize=20&pageNum=1"
        ...
        if "total" in str(res_json["data"]):
            ...
            logger.debug("%s没有历史失信信息" % company_name)
            pass
    except Exception as e:
        logger.debug(e)


def get_dishonest_info(info_id, company_name, tyc_id, pageNum):
    try:
        url = f"https://api4.tianyancha.com/services/v3/past/dishonest?keyWords={company_name}&pageSize=20&pageNum={pageNum}"
        logger.warning(url)
...
def main():
    ...
    for data in data_list:
        ...
        pages_total = get_dishonest_page(info_id, company_name, tyc_id, tyc_hi, Authorization, duid, deviceID)
        if pages_total:
            print(company_name)
            for pageNum in range(1, int(pages_total) + 1):
                get_dishonest_info(info_id, company_name, tyc_id, pageNum)
