...
# -*- coding: utf-8 -*-
"""
@Time : 2023/5/22 13:54
@Author : zhangpf
@File : tyc_historical_final_case.py
@Desc : 历史终本案件
@Software: PyCharm
"""
import sys
...
def get_authoriaztion(info_id, company_name, tyc_id, pageNum):
    ...
    url = HISTORICAL_FINAL_CASE.format(tyc_id, pageNum)
...
def get_historical_final_case_page(
...
def get_historical_final_case_info(info_id, company_name, tyc_id, pageNum):
    try:
        # print(url)
        # res = requests.get(url=url, headers=headers, verify=False).text
        ...
        for ite in items_list:
            item = {
                "info_id": info_id,
                "zname": ite.get("zname", ""),
                "execCourtName": ite.get("execCourtName", ""),
                "caseFinalTime": ite.get("caseFinalTime", ""),
                "caseCreateTime": ite.get("caseCreateTime", ""),
                "caseCode": ite.get("caseCode", ""),
                "execMoney": ite.get("execMoney", ""),
                "noExecMoney": ite.get("noExecMoney", ""),
                "execMoneyUnit": ite.get("execMoneyUnit", ""),
                "noExecMoneyUnit": ite.get("noExecMoneyUnit", ""),
                "businessId": ite.get("businessId", ""),
                "company_name": company_name,
                "tyc_id": tyc_id,
                "create_time": time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(int(time.time()))),
                "url": url,
                "unique_id": str(ite.get("businessId", "")) + "_" + str(tyc_id),
            }
            items.append(item)
        print(items)
        return items
...
def main():
    ...
    for data in data_list:
        ...
        if ex == 1:
            ...
            deviceID = data["data"]["deviceID"]
            x_auth_token = data["data"]["x_auth_token"]
            pages_total = get_historical_final_case_page(
