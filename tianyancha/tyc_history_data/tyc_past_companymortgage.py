#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2022/10/11 15:42
# @Author  : wym
# @File    : tyc_past_companymortgage.py
#!/usr/bin/env python
# -*- coding: utf-8 -*-
...
def create_json(pageNum, info_id, tyc_id, company_name, res_json):
    folder_name = os.getcwd() + "/tyc_past_companyEquity_file_历史动产抵押/"
    ...
    logger.debug(("--tyc_past_companyEquity_file__写入到-->" + file_nm))


def get_authoriaztion(info_id, company_name, tyc_id, pageNum):
    version = "Android 11.4.0"

    url = f"https://api4.tianyancha.com/services/v3/ar/past/companyMortgageV2?name={company_name}&pageSize=20&pageNum={pageNum}"
...
def get_companyMortgage_page(info_id, company_name, tyc_id, tyc_hi, Authorization, duid, deviceID):
    try:
        headers = {
        ...
        url = f"https://api4.tianyancha.com/services/v3/ar/past/companyMortgageV2?name={company_name}&pageSize=20&pageNum=1"
        ...
        if "total" in str(res_json["data"]):
            ...
            logger.debug("%s没有历史动产抵押" % company_name)
            pass
    except Exception as e:
        logger.debug(e)


def get_companyMortgage_info(info_id, company_name, tyc_id, pageNum):
    try:
...
def main():
    ...
    for data in data_list:
        ...
        pages_total = get_companyMortgage_page(info_id, company_name, tyc_id, tyc_hi, Authorization, duid, deviceID)
        if pages_total:
            print(company_name)
            for pageNum in range(1, int(pages_total) + 1):
                get_companyMortgage_info(info_id, company_name, tyc_id, pageNum)
