...
def create_json(pageNum, info_id, tyc_id, company_name, res_json):
    folder_name = os.getcwd() + "/tyc_HistoryAbnormalList_file_历史经营异常/"
    if not os.path.exists(folder_name):
        os.makedirs(folder_name)
    else:
        pass
    ...
    logger.debug(("--tyc_HistoryAbnormalList_file__写入到-->" + file_nm))
...
def get_HistoryAbnormalList_page(info_id, company_name, tyc_id, tyc_hi, Authorization, duid, deviceID, X_AUTH_TOKEN):
    try:
        headers = {
...
def get_HistoryAbnormalList_info(info_id, company_name, tyc_id, pageNum):
    try:
        ...
        X_AUTH_TOKEN = data["data"]["deviceID"]
        headers = {
            "user-agent": "com.tianyancha.skyeye/Dalvik/2.1.0 (Linux; U; Android 8.1.0; Nexus 6P Build/OPM7.181205.001; appDevice/google_QAQ_Nexus 6P)",
            "authorization": Authorization,
            "x-auth-token": X_AUTH_TOKEN,
...
def main():
    ...
    for data in data_list:
        ...
        pages_total = get_HistoryAbnormalList_page(info_id, company_name, tyc_id, tyc_hi, Authorization, duid, deviceID)
        if pages_total:
            print(company_name)
            for pageNum in range(1, int(pages_total) + 1):
                get_HistoryAbnormalList_info(info_id, company_name, tyc_id, pageNum)
