...
from untils.urls import HISTORY_SENIOR_EXECUTIVE
...
def create_json(pageNum, info_id, tyc_id, company_name, res_json):
    folder_name = os.getcwd() + "/jsondata/tyc_历史高管_file/"
    ...
    logger.debug(("--tyc_历史高管_file__写入到-->" + file_nm))


def get_authoriaztion(info_id, company_name, tyc_id, pageNum):
    version = "Android 12.67.0"
    url = HISTORY_SENIOR_EXECUTIVE.format(tyc_id)
...
def get_History_senior_executive_page(
...
def get_History_senior_executive_info(info_id, company_name, tyc_id, pageNum):
    try:
        # print(url)
        # res = requests.get(url=url, headers=headers, verify=False).text
...
def main():
    ...
    for data in data_list:
        ...
        deviceID = data["data"]["deviceID"]
        x_auth_token = data["data"]["x_auth_token"]
        # pages_total = get_History_senior_executive_page(
        #     info_id, company_name, tyc_id, tyc_hi, Authorization, duid, deviceID, x_auth_token
        # )
        # if pages_total:
        #     print(company_name)
        # for pageNum in range(1, int(pages_total) + 1):
        items = get_History_senior_executive_info(info_id, company_name, tyc_id, pageNum)
        try:
            for item in items:
                mq.insert_sql("t_zx_history_senior_executive", item)
        except Exception as e:
            logger.debug(e)
        # else:
        #     pass
