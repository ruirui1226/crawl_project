...
def create_json(pageNum, info_id, tyc_id, company_name, res_json):
    folder_name = os.getcwd() + "/tyc_past_announcement_file_历史开庭公告/"
    if not os.path.exists(folder_name):
        os.makedirs(folder_name)
    else:
        pass
    ...
    logger.debug(("--tyc_past_announcement_file__写入到-->" + file_nm))
...
def get_past_announcement_page(info_id, company_name, tyc_id, tyc_hi, Authorization, duid, deviceID):
    try:
        headers = {
        ...
        if "total" in str(res_json["data"]):
            ...
            logger.debug("%s没有历史开庭公告数据" % company_name)
            pass
    except Exception as e:
        logger.debug(e)


def get_past_announcement_info(info_id, company_name, tyc_id, pageNum):
    try:
        # url = f"https://api4.tianyancha.com/services/v3/past/holder?pageSize=20&id={tyc_id}&pageNum={pageNum}"
        url = f"https://api4.tianyancha.com/services/v3/past/announcement?pageSize=20&id={tyc_id}&pageNum={pageNum}"
        logger.warning(url)
...
def main():
    ...
    for data in data_list:
        ...
        pages_total = get_past_announcement_page(info_id, company_name, tyc_id, tyc_hi, Authorization, duid, deviceID)
        if pages_total:
            print(company_name)
            for pageNum in range(1, int(pages_total) + 1):
                get_past_announcement_info(info_id, company_name, tyc_id, pageNum)
