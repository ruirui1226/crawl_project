...
from untils.urls import HISTORY_SHAREHOLDER

requests.packages.urllib3.disable_warnings(InsecureRequestWarning)


def create_json(pageNum, info_id, tyc_id, company_name, res_json):
    folder_name = os.getcwd() + "/jsondata/tyc_历史股东_file/"
    ...
    logger.debug(("--tyc_历史股东_file__写入到-->" + file_nm))


def get_authoriaztion(info_id, company_name, tyc_id, pageNum):
    version = "Android 12.67.0"
    url = HISTORY_SHAREHOLDER.format(tyc_id, pageNum)
...
def get_shareholder_page(
    ...
    try:
        ...
        #     "status": "-100",
        # }
        url = HISTORY_SHAREHOLDER.format(tyc_id, initial_pageNum)
...
def get_shareholder_info(info_id, company_name, tyc_id, pageNum):
    try:
        # print(url)
        # res = requests.get(url=url, headers=headers, verify=False).text
        ...
        for re in res_json["data"]["result"]:
            item = {
                # "id": str(tyc_id) + re.get("id", ""),
                "info_id": info_id,
                # "tagList": ",".join(format(log) for log in re.get("tagList")) if re.get("tagList") is not None else "",
                "tagList": str(re.get("tagList", "")).replace("'",'"'),
                "toco": re.get("toco", ""),
                "amount": re.get("amount", ""),
                "logo": re.get("logo", ""),
                "alias": re.get("alias", ""),
                # "capital": ",".join(format(log) for log in re.get("capital")) if re.get("capital") is not None else "",
                "capital": str(re.get("capital", "")).replace("'",'"'),
                "name": re.get("name", ""),
                "startTime": re.get("startTime", ""),
                "t_id": re.get("id", ""),
                "endTime": re.get("endTime", ""),
                "type": re.get("type", ""),
...
def main():
    ...
    for data in data_list:
        ...
        deviceID = data["data"]["deviceID"]
        x_auth_token = data["data"]["x_auth_token"]
        pages_total = get_shareholder_page(
            info_id, company_name, tyc_id, tyc_hi, Authorization, duid, deviceID, x_auth_token, initial_pageNum
        )
        if pages_total:
            print(company_name)
            for pageNum in range(1, int(pages_total) + 1):
                items = get_shareholder_info(info_id, company_name, tyc_id, pageNum)
                try:
                    mq = MysqlPipelinePublic()
                    for item in items:
                        mq.insert_sql("t_zx_history_shareholder", item)
                    mq.close()
