...
def create_json(pageNum, info_id, tyc_id, company_name, res_json):
    folder_name = os.getcwd() + "/tyc_zhixing_file_历史被执行人/"
    if not os.path.exists(folder_name):
        os.makedirs(folder_name)
    else:
        pass
    ...
    logger.debug(("--tyc_zhixing_file__写入到-->" + file_nm))


def get_authoriaztion(info_id, company_name, tyc_id, pageNum):
    version = "Android 11.4.0"

    url = f"https://api4.tianyancha.com/services/v3/ar/past/zhixing?pageSize=20&id={tyc_id}&pageNum={pageNum}"
...
def get_hixing_page(info_id, company_name, tyc_id, tyc_hi, Authorization, duid, deviceID):
    try:
        headers = {
        ...
        url = f"https://api4.tianyancha.com/services/v3/ar/past/zhixing?pageSize=20&id={tyc_id}&pageNum=1"
...
def get_hixing_info(info_id, company_name, tyc_id, pageNum):
    try:
        url = f"https://api4.tianyancha.com/services/v3/ar/past/zhixing?pageSize=20&id={tyc_id}&pageNum={pageNum}"
        logger.warning(url)
...
def main():
    ...
    for data in data_list:
        ...
        pages_total = get_hixing_page(info_id, company_name, tyc_id, tyc_hi, Authorization, duid, deviceID)
        if pages_total:
            print(company_name)
            for pageNum in range(1, int(pages_total) + 1):
                get_hixing_info(info_id, company_name, tyc_id, pageNum)
