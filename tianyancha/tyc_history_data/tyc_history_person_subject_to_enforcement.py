...
from untils.urls import HISTORY_PERSON_SUBJECT_TO_ENFORCEMENT
...
def create_json(pageNum, info_id, tyc_id, company_name, res_json):
    folder_name = os.getcwd() + "/jsondata/tyc_历史被执行人_file/"
...
def get_authoriaztion(info_id, company_name, tyc_id, pageNum):
    ...
    url = HISTORY_PERSON_SUBJECT_TO_ENFORCEMENT.format(tyc_id, pageNum)
...
def get_person_subject_to_enforcement_page(
...
def get_person_subject_to_enforcement_info(info_id, company_name, tyc_id, pageNum):
    try:
...
def main():
    ...
    for data in data_list:
        ...
        deviceID = data["data"]["deviceID"]
        x_auth_token = data["data"]["x_auth_token"]
        pages_total = get_person_subject_to_enforcement_page(
            info_id, company_name, tyc_id, tyc_hi, Authorization, duid, deviceID, x_auth_token, initial_pageNum
        )
        if pages_total:
            print(company_name)
            for pageNum in range(1, int(pages_total) + 1):
                items = get_person_subject_to_enforcement_info(info_id, company_name, tyc_id, pageNum)
                try:
                    for item in items:
                        mq.insert_sql("t_zx_history_person_subject_to_enforcement", item)
                except Exception as e:
                    logger.debug(e)
        else:
            pass
