# -*- coding: utf-8 -*-
"""
Created on 2023-06-12 10:26:03
---------
@summary:
---------
@author: zhongxing
...
from items.t_zx_tendering_shandong_taian_seed_task_item import TZxTenderingShandongTaianSeedTaskItem


class TenderingTaianChannelTaks(feapder.BatchSpider):
    ...
    def parse(self, request, response):
        ...
        for panel in panel_list:
            ...
            for nav_link in nav_links:
                for page_i in range(0, 45, 15):
                    two_title = nav_link.xpath("./text()").extract_first()
                    url = nav_link.xpath("./@href").extract_first()
                    post_url = 'http://www.taggzyjy.com.cn/inteligentsearch/rest/esinteligentsearch/getFullTextDataNew'
                    data_rid = re.findall('.*/(.*?)/notice_.*', url)
                    if data_rid:
                        data_id = data_rid[0]
                    item = TZxTenderingShandongTaianSeedTaskItem()
                    item.url = url
...
if __name__ == "__main__":
    spider = TenderingTaianChannelTaks(
        # task_id=1,
        redis_key="tendering_crawler:shandong_taian_channel_task",  # 分布式爬虫调度信息存储位置
        task_table="t_zx_tendering_shandong_taian_channel_task",  # mysql中的任务表
        task_keys=["id", "url"],  # 需要获取任务表里的字段名，可添加多个
        task_state="state",  # mysql中任务状态字段
        batch_record_table="tanan_channel_batch_record",  # mysql中的批次记录表
        batch_name="获取种子任务",  # 批次名字
        batch_interval=1,  # 批次周期 天为单位 若为小时 可写 1 / 24
    )

    # parser = ArgumentParser(description="TenderingTaianChannelTaks爬虫")
    #
    # parser.add_argument(
    #     "--start_master",
    #     action="store_true",
    ...
    # 通过命令行启动
    # python tendering_taian_channel_taks.py --start_master  # 添加任务
    # python tendering_taian_channel_taks.py --start_worker  # 启动爬虫
